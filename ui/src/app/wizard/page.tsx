...
export default function WizardPage() {
  ...
  const isLoading = authLoading || passportLoading;
  ...
  useEffect(() => {
    if (!isLoading && currentPassport?.wizardComplete) {
      router.push('/dashboard');
    }
  }, [currentPassport, isLoading, router]);
  ...
  const handleAddBullet = async (section: SectionType, content: string, remedialSuggestion?: string) => {
    if (passportId) {
      // For now, we add remedial suggestion as part of content if provided
  ...
  const handleCompleteWizard = async () => {
    ...
    // Navigation happens in the WizardFlow component
  };
        ...
        <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
          {!isFirstStep && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 min-h-[44px] text-sm font-medium text-gray-900 hover:bg-gray-100 rounded-md transition-all focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-600 disabled:opacity-50"
            >
              ← Back
            </button>
          )}
